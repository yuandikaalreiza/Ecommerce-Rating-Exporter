/* A compact, dependency-free XLSX writer. It creates a standards-compliant ZIP
 * container with one worksheet, using inline strings to preserve order IDs. */
(() => {
  ...
  function cell(reference, value, style) {
    ...
    return `<c r="${reference}" t="inlineStr"${styleAttribute}><is><t xml:space="preserve">${xml(value)}</t></is></c>`;
  }
  ...
  function workbook(columns, rows) {
    const sheetRows = [columns, ...rows].map((row, rowIndex) => {
      ...
      const cells = values.map((value, columnIndex) => cell(`${columnName(columnIndex)}${rowIndex + 1}`, value, rowIndex === 0 ? 1 : 0)).join("");
    ...
    return new Blob([zip([
      ...
      { name: "xl/workbook.xml", content: `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="Shop Ratings" sheetId="1" r:id="rId1"/></sheets></workbook>` },
  ...
  function createUrl(columns, rows) {
    const url = URL.createObjectURL(workbook(columns, rows));
    return url;
  }

  globalThis.ShopeeXlsx = { createUrl };
})();
